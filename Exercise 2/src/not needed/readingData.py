...
def rd_breastCancer(dataset_location, lines, testing_ratio, random_state):
	df = pd.read_csv(dataset_location + "breast-cancer-diagnostic.shuf.lrn.csv", nrows=lines)
	data_training, data_testing = train_test_split(df, test_size=testing_ratio, random_state=random_state)
...
def rd_purchase(dataset_location, lines, testing_ratio, random_state):
	df = pd.read_csv(dataset_location + "purchase600-100cls-15k.lrn.csv", nrows=lines)
	data_training, data_testing = train_test_split(df, test_size=testing_ratio, random_state=random_state)
	return data_training, data_testing, None # None because everything is ratio data, so it is not needed

def rd_speeddating(dataset_location, lines, testing_ratio, random_state):
	df = pd.read_csv(dataset_location + "speeddating.csv", nrows=lines)
